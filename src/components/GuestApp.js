...
import GuestTodos from "./GuestTodos";
import {supabase} from "./supabaseClient";
import { useNavigate } from 'react-router-dom';
import {Container, Row, Col, Button, Form} from "react-bootstrap";

function GuestApp({ user }) {
    ...
    return (
        <Container id="GuestAppBu" fluid className="p-3" style={{ height: "100vh" }}>
            {/* Navbar */}
            <div style={{
                width: "100%",
                height: "60px",
            ...
            <Row className="h-100">
                <Col md={4} className="border-end overflow-auto">
                    <GuestList selectedList={selectedList} onSelectList={setSelectedList} user={user} />
                </Col>
                <Col md={8} className="overflow-auto">
                    <GuestTodos selectedList={selectedList} user={user} />
